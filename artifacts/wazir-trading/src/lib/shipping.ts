...
export function computeLandedCost(
  ...
  const fob = fobUsd ?? 0;
  const freight = freightType === 'Prepaid' ? rate.freight_usd : 0;
  const inspection = withInspection ? rate.inspection_fee : 0;
  const insurance = withInsurance ? fob * rate.insurance_rate : 0;
