...
/* ═══════════════════════════════════════════════════════════════
   PASSWORD GATE
═══════════════════════════════════════════════════════════════ */
function PasswordGate({ onAuth }: { onAuth: (password: string, cloudName: string) => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/admin/cloudinary/auth', {
      ...
      if (!res.ok) {
        setError(
          res.status === 401
            ...
            : res.status === 503
              ? (body.error ?? 'Cloudinary/admin access is not configured on the server.')
              : (body.error ?? `Admin server unavailable (HTTP ${res.status}).`),
        );
        setPw('');
        return;
      }
      ...
      onAuth(pw, body.cloud_name);
    } catch {
      setError('Unable to reach the admin server. Try again.');
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-[#C8102E] hover:bg-[#a00d24] disabled:opacity-60 text-white font-semibold py-2.5 rounded-lg transition-colors"
          >
...
export default function AdminBulkUpload() {
  ...
  const authenticate = (password: string, authenticatedCloudName: string) => {
    adminPassword = password;
    cloudName = authenticatedCloudName;
    setAuthed(true);
  };
  const logout = () => { adminPassword = ''; setAuthed(false); };
  ...
  if (!authed) return <PasswordGate onAuth={authenticate} />;
