...
import {
  ...
  Star, Trash2, Download, ChevronDown, ChevronUp, Lock, Search,
  PackageOpen, FileSpreadsheet, FolderArchive, RefreshCw, Camera,
  GripVertical, X,
} from 'lucide-react';

/* ─── CONSTANTS ──────────────────────────────────────────────── */
// Cloudinary cloud name is public configuration. Prefer the authenticated API response so
// the admin tool works without a separate VITE_CLOUDINARY_CLOUD_NAME frontend variable.
let cloudName = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME || '';

/** Resolves the Cloudinary cloud name from the server when it isn't set locally. */
async function ensureCloudName(): Promise<string> {
  if (cloudName) return cloudName;
  try {
    const res = await fetch('/api/public/cloudinary/auth');
    const body = await res.json().catch(() => ({})) as { cloud_name?: string };
    if (body.cloud_name) cloudName = body.cloud_name;
  } catch {
    /* ignore — upload will surface a clear error below */
  }
  return cloudName;
}
// Cloudinary Admin API calls are proxied through the api-server to keep secrets server-side.
// The frontend no longer needs a password prompt; server-side Cloudinary credentials remain protected.
...
const API_BASE = '/api/public/cloudinary';
...
async function listSubfolders(dateFolder: string): Promise<string[]> {
  const data = await adminFetch(
    `/folders/${encodeURIComponent(dateFolder)}`
  );
...
async function listFolderImages(dateFolder: string, subfolder: string): Promise<{ url: string; public_id: string }[]> {
  const data = await adminFetch(
    `/resources/${encodeURIComponent(dateFolder)}/${encodeURIComponent(subfolder)}`
  );
...
async function fetchFlatFolder(
  ...
  do {
    page++;
    const url = new URL('/api/public/cloudinary/fetch-flat-folder', window.location.origin);
...
async function searchCloudinaryByChassis(
  ...
  const res = await fetch('/api/public/cloudinary/search-by-chassis', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
async function saveCloudinaryImagesToDb(
  ...
  const { data: existing, error: existingError } = await supabase
    ...
    .select('image_url')
    .eq('car_id', carId);
  if (existingError) return { saved: 0, error: existingError.message };

  const existingUrls = new Set(
    (existing ?? []).map((image: { image_url: string }) => image.image_url),
  );
  const newImages = images.filter(image => !existingUrls.has(image.url));
  if (!newImages.length) return { saved: 0 };
